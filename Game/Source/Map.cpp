...
bool Map::Load(SString mapFileName)
{
    ...
    if(result == NULL)
    {
       ...
       // Iterate the Tileset
        ...
        for (pugi::xml_node hitbox = mapFileXML.child("map").child("objectgroup").child("object"); hitbox != NULL; hitbox = hitbox.next_sibling("object")) {
            std::string s = hitbox.child("polygon").attribute("points").as_string();
            std::istringstream iss(s);
            std::string point;
            int x = hitbox.attribute("x").as_int();
            int y = hitbox.attribute("y").as_int();
